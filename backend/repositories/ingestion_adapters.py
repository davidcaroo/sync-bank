...
class SyncFacturaRepositoryAdapter(FacturaRepositoryPort):
    ...
    async def save_factura(
        self, factura_payload: dict[str, Any], items: list[dict[str, Any]]
    ) -> dict[str, Any]:
        return await self._run_in_executor(
            lambda: self._save_factura(factura_payload, items)
        )
...
class SyncProviderConfigRepositoryAdapter(ProviderConfigRepositoryPort):
    ...
    async def get_config_cuenta(
        self, nit_proveedor: str | None
    ) -> dict[str, Any] | None:
        return await self._run_in_executor(
            lambda: self._get_config_cuenta(nit_proveedor)
        )

    async def sync_proveedor_nombre(
        self, nit_proveedor: str | None, nombre_proveedor: str | None
    ) -> None:
        await self._run_in_executor(
            lambda: self._sync_config_proveedor_nombre(nit_proveedor, nombre_proveedor)
        )
