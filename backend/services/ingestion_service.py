...
class IngestionService:
        ...
        for file in files or []:
            ...
            if lower_name.endswith(".xml"):
                ...
                if xml_text is None:
                    ...
                    continue
                documents.append(XMLDocument(file_name=filename, entry_name=filename, xml_text=xml_text))
                continue

            if lower_name.endswith(".zip"):
                zip_entries, zip_errors = self._extract_xml_from_zip_bytes(filename, content)
                documents.extend(zip_entries)
    ...
    def _extract_xml_from_zip_bytes(self, zip_name: str, content: bytes) -> tuple[list[XMLDocument], list[dict]]:
        documents: list[XMLDocument] = []
        errors: list[dict] = []
            ...
            with zipfile.ZipFile(io.BytesIO(content)) as zipped:
                for entry_name in zipped.namelist():
                    if not entry_name.lower().endswith(".xml"):
                        continue
                    try:
                        xml_bytes = zipped.read(entry_name)
                        xml_text = self._decode_xml_bytes(xml_bytes)
                        if xml_text is None:
                            errors.append(
                                {
                                    "file_name": zip_name,
                            ...
                            continue
                        documents.append(XMLDocument(file_name=zip_name, entry_name=entry_name, xml_text=xml_text))
                    except Exception as exc:
                        errors.append(
                            {
                                "file_name": zip_name,
